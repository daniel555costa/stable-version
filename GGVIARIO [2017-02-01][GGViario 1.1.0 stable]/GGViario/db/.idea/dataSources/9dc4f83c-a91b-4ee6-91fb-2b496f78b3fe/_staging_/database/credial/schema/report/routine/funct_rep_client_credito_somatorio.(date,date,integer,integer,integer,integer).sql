CREATE OR REPLACE FUNCTION report.funct_rep_client_credito_somatorio (
  iduser CHARACTER varying,
  idagencia integer,
  "dataInicio" date,
  "dataFim" date,
  filter json
) RETURNS TABLE("NIF" character varying, "NAME" character varying, "SURNAME" character varying, "VALOR" character varying, "LOCAL TRABALHO" character varying)
	LANGUAGE plpgsql
AS $$
    DECLARE
      ...
      totalSomatorio DOUBLE PRECISION DEFAULT 0;
      i RECORD;
    ...
    BEGIN
      ...
      FOR I IN (
        ...
        IF i.valorCredito IS NOT NULL THEN
          ...
          totalSomatorio := totalSomatorio + i.valorCredito;
          RETURN NEXT;
