-- DROP FUNCTION  rule.getChequeRandom ("idUser" character varying, "idBanco" numeric, "idAgencia" numeric, "valorRequisicao" double precision);
CREATE OR REPLACE FUNCTION rule.get_cheque_randon ("idUser" character varying, idconta numeric, "idAgencia" numeric, "valorRequisicao" double precision) RETURNS chequempresa
	LANGUAGE plpgsql
AS $$
  DECLARE
    ...
    vCheque credial.chequempresa;
    vChequeAux credial.chequempresa;
    ...
    if tt != 0 and saldoSuf = 1 then
      ...
      select * into vCheque
        from credial.chequempresa ch
          ...
          inner join credial.banco bc on ch.cheq_banco_id = bc.banco_id
