create or REPLACE PACKAGE BODY PACK_CONTA AS
  ...
  FUNCTION func_reg_payemnt (idUser NUMBER,
    ...
    */
      ...
      SELECT SUM(IP.IPAY_VALUE) INTO sumDebito
         FROM T_ITEMPAYMENT IP
         WHERE IP.IPAY_PAY_ID = idPayment
            AND IP.IPAY_TMOV_ID = 1;
            
      SELECT SUM(IP.IPAY_VALUE) INTO sumCredito
         FROM T_ITEMPAYMENT IP
         WHERE IP.IPAY_PAY_ID = idPayment
            AND IP.IPAY_TMOV_ID = 2;
